...
if __name__ == '__main__':
    file_list_file = './missing_1_species_list.txt'
    file_list = np.loadtxt(file_list_file, dtype = str)

    species_list = ['Human', 'Chimp', 'Goril', 'Orang', 'Macaq', 'Mouse']

    if not os.path.isdir('./prepared_input1/'):
        os.mkdir('./prepared_input1')
        
    for seq_file in file_list:
        row_num = 0
        with open('./intronAlignmentSeparated/' + seq_file, 'r') as f:
            with open('./prepared_input1/' + seq_file.replace('.', '_').replace('_pos_seq_formatted', '.fasta'), 'w+') as g:
                with open('./prepared_input1/' + \
